...
function save_checklist_item_template(e,t){
    ...
    var ar=$('.checklist[data-checklist-id="'+e+'"]  .text-ar').val();
    ar=ar.trim();
    var en=$('.checklist[data-checklist-id="'+e+'"]  .text-en').val();
    en=en.trim();
    $.post(admin_url+"tasks/save_checklist_item_template",
    {
        description:a ,  description_ar:ar , description_en:en  }).done((
            function(e){
...
function update_checklist_order(){var e=[],t=$("body").find(".checklist");if(0!==t.length){var a=1;$.each(t,(function(){e.push([$(this).data("checklist-id"),a]),a++}));var i={};i.order=e,$.post(admin_url+"tasks/update_checklist_order",i)}}
function add_task_checklist_item(e,t,a){
    a&&$(a).addClass("disabled"),t=void 0===t?"":t,
    $.post(admin_url+"tasks/add_checklist_item",{
        taskid:e,description_en:t
    }).done((function(){
...
function update_task_checklist_item(e){
    ...
    var t=$.Deferred();return setTimeout((function(){
        var a=e.val();a=a.trim();
        var i=e.parents(".checklist").data("checklist-id");

        var dar_val= e.parents(".checklist").find("textarea.text-ar").val();
        dar_val=dar_val.trim();
        var den_val=e.parents(".checklist").find("textarea.text-en").val();
        den_val=den_val.trim();
        $.post(admin_url+"tasks/update_checklist_item",{
            description_en:den_val,description_ar:dar_val,listid:i}).done((function(n){
                t.resolve(),!0===(n=JSON.parse(n))
                .can_be_template&&e.parents(".checklist").find(".save-checklist-template").removeClass("hide"),""===a&&$("#checklist-items").find('.checklist[data-checklist-id="'+i+'"]').remove()
            }))
        }),300),t.promise()
    }
function delete_checklist_item(e,t){
    var checklistcar = $(t).parents(".checklist").data('checklistcar-id');

    requestGetJSON("tasks/delete_checklist_item/"+e)
    .done((function(e){
        // if(checklistcar != ''){
        //     return_car_checklist_item(checklistcar , e );
        // }
...
function add_task_to_preview_as_item(e){
    requestGetJSON("tasks/get_billable_task_data/"+e).done((function(t){
        ...
        a.find('input[name="task_id_car"]').val(t.customer_car),
        a.find('input[name="quantity"]').val(t.total_hours),
        a.find('input[name="rate"]').val(t.hourly_rate),
        a.find('input[name="unit"]').val(""),$('input[name="task_id"]').val(e),$(document).trigger({type:"item-added-to-preview",item:t,item_type:"task"})}))
    }
    function clear_item_preview_values(e){var t=$("table.items tbody").find("tr:last-child").find("select").selectpicker("val"),a=$(".main");a.find("textarea").val(""),a.find('td.custom_field input[type="checkbox"]').prop("checked",!1),a.find("td.custom_field input:not(:checkbox):not(:hidden)").val(""),a.find("td.custom_field select").selectpicker("val",""),a.find('input[name="quantity"]').val(1),a.find("select.tax").selectpicker("val",t),a.find('input[name="rate"]').val(""),a.find('input[name="unit"]').val(""),$('input[name="task_id"]').val(""),$('input[name="expense_id"]').val("")
    }
